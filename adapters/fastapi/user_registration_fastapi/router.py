...
def register_user(
    payload: RegisterUserRequest, service: RegistrationServiceDependency
    ...
    if result.status is RegistrationStatus.DUPLICATE:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT, detail=list(result.errors)
        )
    ...
    return RegisterUserResponse(user_id=result.user_id)
